...
for L in [10**7]:#
    for r in [1]:
        ...
        for x in range(1,11):
            ...
            #ts.print_history()
            ts=msprime.sim_ancestry(samples=sample_size,recombination_rate=r,random_seed=((x+3)**2),sequence_length=L,demography=demography,ploidy=2 ,model=[msprime.DiscreteTimeWrightFisher(duration=1000),msprime.StandardCoalescent()])
            mts = msprime.sim_mutations(ts, rate=mu)
            f = open("Sup_Table_1_small_Increase_mu"+str(mu)+'r'+str(r)+'x'+str(x)+'L'+str(L)+".txt","w")
